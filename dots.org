...
(general-define-key
 ...
 "<f5>" '(lambda()
           (interactive)
           (async-shell-command
            (concat "guile " (buffer-file-name)) "*guile output*")))
#+end_src
...
:straight (pdf-continuous-scroll-mode.el :type git :host github :repo "dalanicolai/pdf-continuous-scroll-mode.el"))
#+end_src
...
(use-package lispyville
  ...
  :config
  (lispyville-set-key-theme '(operators additional-insert c-w additional text-objects atom-motions additional-motions wrap slurp/barf-lispy)))
#+end_src
...
*** org-pdftools
Allows to make org-link to a specific page in a pdf.
#+begin_src emacs-lisp
(use-package org-pdftools
  :hook (org-mode . org-pdftools-setup-link)
  :custom
  (org-pdftools-root-dir "~/Documents/books")
...
(use-package ligature
  :straight (lsp-mode :type git :host github :repo "mickeynp/ligature.el")
  :config
...
(use-package exwm
  ...
  (setq exwm-input-global-keys
        `(
          ([?\s-q] . ram/switch-to-previous-buffer)
...
(defun ram/dired-open()
  ...
  (cond
   ...
   ((string= ext "rar")
    (async-start-process "File-roller" "file-roller" nil file))
   ((string= ext "tar")
    (async-start-process "File-roller" "file-roller" nil file))
   ((string= ext "bz2")
    (async-start-process "File-roller" "file-roller" nil file))
   ...
   ((string= ext "gz")
    (async-start-process "File-roller" "file-roller" nil file))
   ((string= ext "mp4")
    (async-start-process "Mpv" "mpv" nil file))
   ((string= ext "mkv")
    (async-start-process "Mpv" "mpv" nil file))
   ((string= ext "mp3")
...
*** Gcc
#+begin_src sh
export PATH="$HOME/bin/gcc-10.2.0/bin:$PATH"
